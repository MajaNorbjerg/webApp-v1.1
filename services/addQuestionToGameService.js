...
class AddQuestionToGameService {
    ...
    showList() {
        let select = document.querySelector('#select-games');
        console.log('hi');
        let listItem = ""
        let listItems = "";
        this.questions.forEach(question => {
            ...
            if (question.game == select.value) { // Checks if the question is a part of the "current" game
        ...
        this.gameRef.doc(`${select.value}`).get().then(function (doc) { // Get data for the selected game from the database-gameId-document
            let game = doc.data()
