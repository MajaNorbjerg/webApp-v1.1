...
export default class AddQuestions {
  constructor() {
    ...
    this.template();
    this.createGameOptions('select-game');
    this.createGameOptions('select-games');
  ...
  template() {
    ...
    <section id="addQuestions" class="page">
      ...
      <form id="questionForm">
        <h2>Tilføj dine egne spørgsmål til spillet:</h2>
      ...
      <article id="appendUserQuestions">
        ...
        <select class="inputfield" id="select-games" name="games"s onchange="showList()" placeholder="Vælg spil..." required></select>
        <div id="list">
        </div>
        <div id="predefined">
        </div>
  ...
  createGameOptions(elementId) {
    this.gameRef.get().then(snapshotData => {
      snapshotData.forEach(doc => {
        let game = doc.data();
        game.id = doc.id;

        let listOfGames = document.getElementById(`${elementId}`);
  ...
  basket() {
    ...
    document.querySelector('#numberOfAdded').innerHTML = `<p>Du har tilføjet <b>${_arrayNewQuestionService.partyContentArray.length}</b> spørgsmål til spillet</p>`
